            ...
            <ScrollView
                ...
                {!loading && equipmentData && (<View style={{ backgroundColor: theme.colors.surface, marginBottom: 10, padding: 10 }}>
                    ...
                    <View style={{marginVertical: 20}}>
                        {selectedSlot && (
                            <Text style={{ textAlign: 'center', color: theme.colors.onBackground, fontWeight: 'bold', fontSize: 16, marginVertical: 10 }}>{dayjs(selectedSlot).format('[Date:] DD-MM-YYYY & [Slot:] hh:mm A')}</Text>
                        )}
                        <Button
                            disabled={!selectedSlot}
                            mode="contained"
                            style={{ marginBottom: 10, fontSize: 18, paddingVertical: 5 }}
                            onPress={() => {
                                router.push({
